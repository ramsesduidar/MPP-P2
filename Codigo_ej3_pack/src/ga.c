...
double aplicar_ga(int name, int p, const double *d, int n, int n_gen, int tam_pob, double m_rate, int ngm, int nem, int *sol)
{
	...
	if (name==0){
		...
		for(int dest = 1; dest < p; dest++){
			...
			MPI_Pack(poblacion[j+new_tam*dest],sizeof(Individuo),MPI_BYTE,trozo,sizeof(Individuo)*new_tam,&posicion,MPI_COMM_WORLD);
			...
			// Y los enviamos.
			MPI_Send(trozo, sizeof(Individuo)*new_tam, MPI_PACKED, dest, tag, MPI_COMM_WORLD);
	...
	// El resto de procesos
		...
		// Y recibe
		MPI_Recv(trozo, sizeof(Individuo)*new_tam, MPI_PACKED, 0, tag, MPI_COMM_WORLD, &status);
		
		for(int j = 0; j < new_tam; j++){
			// Guardamos en el array punteros a los individuos en trozo[].
			//poblacion[j] = &trozo[j];
			poblacion[j] = (Individuo *) malloc(sizeof(Individuo));
			MPI_Unpack(trozo,sizeof(Individuo)*new_tam,&posicion,poblacion[j],sizeof(Individuo),MPI_BYTE,MPI_COMM_WORLD);
		...
		if (n_iteraciones == ngm && g!= n_gen-1){
			...
			if (name == 0){
				...
				for(int msg = 1; msg < p; msg++){
					...
					MPI_Recv(trozo, sizeof(Individuo)*new_tam, MPI_PACKED, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &status);
					
					for (int i = 0; i<nem; i++){
						// Y lo copiamos a partir de la posicion new_tam
						//memcpy(poblacion[pos], &trozo[i], sizeof(Individuo));
						MPI_Unpack(trozo, sizeof(Individuo)*new_tam, &posicion, poblacion[pos], sizeof(Individuo), MPI_BYTE, MPI_COMM_WORLD);
				...
				for(int j = 0; j < nem*(p-1); j++){
					...
					MPI_Pack(poblacion[j],sizeof(Individuo),MPI_BYTE,mejores,sizeof(Individuo)*nem*(p-1),&posicion,MPI_COMM_WORLD);
				}
				// Y enviamos a cada isla nem individuos
				tag = 4;
				for (int dest = 1; dest < p; dest++){
					MPI_Send(&mejores[nem*(dest-1)], sizeof(Individuo)*nem, MPI_PACKED, dest, tag, MPI_COMM_WORLD);
				}
				free(mejores);
			...
			// El resto de procesos
				...
				// Como el array poblacion tiene una copia de los individuos recibidos en trozo[]
				// sólo tenemos que copiar de población a trozo.
				int posicion = 0;
				for(int i = 0; i<new_tam; i++){
					//memcpy(&trozo[i], poblacion[i], sizeof(Individuo));
					MPI_Pack(poblacion[i],sizeof(Individuo),MPI_BYTE,trozo,sizeof(Individuo)*new_tam,&posicion,MPI_COMM_WORLD);
				}
				int tag = 3;
				MPI_Send(trozo, sizeof(Individuo)*nem, MPI_PACKED, 0, tag, MPI_COMM_WORLD);
				
				
				tag = 4;
				// Recibimos nem individuos, sustituyendolos por los nem primeros individuos de trozo[]
				// que son los que acabamos de enviar.
				MPI_Recv(trozo, sizeof(Individuo)*nem, MPI_PACKED, 0, tag, MPI_COMM_WORLD, &status);
				posicion = 0;
				// Desempaquetamos
				for (int i = 0; i<nem; i++){
					// Y lo copiamos a partir de la posicion new_tam
					MPI_Unpack(trozo, sizeof(Individuo)*new_tam, &posicion, poblacion[i], sizeof(Individuo), MPI_BYTE, MPI_COMM_WORLD);
	...
	if (name == 0){
		...
		for(int msg = 1; msg < p; msg++){
			...
			MPI_Recv(trozo, sizeof(Individuo), MPI_PACKED, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &status);
			...
			if (poblacion[0]->fitness > trozo[0].fitness)
				...
				MPI_Unpack(trozo, sizeof(Individuo)*new_tam, &posicion, poblacion[0], sizeof(Individuo), MPI_BYTE, MPI_COMM_WORLD);
		}
	...
	// El resto los envia, poblacion[0] que es el mejor.
		...
		int tag = 5;
		int posicion = 0;
		MPI_Pack(poblacion[0],sizeof(Individuo),MPI_BYTE,trozo,sizeof(Individuo),&posicion,MPI_COMM_WORLD);
		MPI_Send(trozo, sizeof(Individuo), MPI_PACKED, 0, tag, MPI_COMM_WORLD);
	}
