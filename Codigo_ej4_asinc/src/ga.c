...
double aplicar_ga(int name, int p, const double *d, int n, int n_gen, int tam_pob, double m_rate, int ngm, int nem, int *sol)
{
	...
	//printf("Entramos a ga: new_tam = %d\n", new_tam);
	// El proceso 0
	if (name==0){
		...
		// Creamos un buffer de tamaño (tam_pob-new_tam) para las comunicaciones
		trozo = (Individuo *) malloc((tam_pob-new_tam) * sizeof(Individuo));
		...
		for(i = new_tam; i < tam_pob; i++) {
			memcpy(&trozo[i-new_tam], poblacion[i], sizeof(Individuo));
		...
		for(int dest = 1; dest < p; dest++){
			...
			// Y los enviamos.
			MPI_Isend(&trozo[new_tam*(dest-1)], new_tam, individuo_type, dest, tag, MPI_COMM_WORLD, &requests[dest]);
		}
	}
	// El resto de procesos
		...
		// Y recibe
		MPI_Irecv(trozo, new_tam, individuo_type, 0, tag, MPI_COMM_WORLD, &requests[0]);
		...
		if (n_iteraciones == ngm && g!= n_gen-1){
			...
			if (name == 0){
				...
				for(int src = 1; src < p; src++){
					...
					MPI_Irecv(&trozo[new_tam*(src-1)], nem, individuo_type, src, tag, MPI_COMM_WORLD, &requests[src]);
					
				}
				
				int pos = new_tam;
				for(int src = 1; src < p; src++){
					// Esperamos a que nos lleguen los datos
					MPI_Wait(&requests[src], &status);
					
					for (int i = 0; i<nem; i++){
						// Y lo copiamos a partir de la posicion new_tam
						memcpy(poblacion[pos], &trozo[new_tam*(src-1)+i], sizeof(Individuo));
						//printf("fitness recibido: %lf\n", trozo[new_tam*(src-1)+i].fitness);
						pos++;
					}
				...
				// guardamos en trozo[] los nem*(p-1) mejores
				...
				tag = 4;
				for (int dest = 1; dest < p; dest++){
					MPI_Isend(&trozo[nem*(dest-1)], nem, individuo_type, dest, tag, MPI_COMM_WORLD, &requests[dest]);
				}
			...
			// El resto de procesos
				...
				// Copiamos los nem mejores en el array trozo.
				for(int i = 0; i<nem; i++){
					memcpy(&trozo[i], poblacion[i], sizeof(Individuo));
				}
				int tag = 3;
				MPI_Isend(trozo, nem, individuo_type, 0, tag, MPI_COMM_WORLD, &requests[0]);
				
				// No es necesario, ya que p0 nos enviará los datos despues de recibir los nuestros
				//MPI_Wait(&requests[0], &status);
				
				tag = 4;
				// Recibimos nem individuos, sustituyendolos por los nem primeros individuos de trozo[]
				// que son los que acabamos de enviar.
				MPI_Irecv(trozo, nem, individuo_type, 0, tag, MPI_COMM_WORLD, &requests[0]);
				
				// Este wait si que es necesario
				MPI_Wait(&requests[0], &status);
				for(int i = 0; i < nem; i++){
					// Guardamos en el array punteros a los individuos en trozo[].
	...
	if (name == 0){
		...
		for(int src = 1; src < p; src++){
			...
			MPI_Irecv(&trozo[src], 1, individuo_type, src, tag, MPI_COMM_WORLD, &requests[src]);
		}
		...
		for(int src = 1; src < p; src++){
			...
			if (poblacion[0]->fitness > trozo[src].fitness)
				memcpy(poblacion[0], &trozo[src], sizeof(Individuo));
			//pos++;
		}
	...
	// El resto los envia, poblacion[0] que es el mejor.
		...
		int tag = 5;
		MPI_Isend(poblacion[0], 1, individuo_type, 0, tag, MPI_COMM_WORLD, &requests[0]);
		
		// Esperamos para no hacer free antes de tiempo
		MPI_Wait(&requests[0], &status);
	}
